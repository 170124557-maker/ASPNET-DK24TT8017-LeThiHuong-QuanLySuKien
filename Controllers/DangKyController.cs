...
namespace DA09_QLSK.Controllers
{
    ...
    {
        // Chỉ khai báo _context MỘT LẦN DUY NHẤT ở đây
        ...
        public IActionResult DangKyThamGia(string maSK)
        {
            ...
            // 2. Nếu chưa đăng nhập -> Thông báo và bắt buộc quay về trang Chi tiết
            ...
            // 3. Nếu đã đăng nhập -> Xử lý lưu đăng ký vào DB
            // (Ở đây bạn gọi hàm lưu vào Database của bạn, ví dụ: _context.DangKy.Add(...))

            // Sau khi lưu thành công:
            TempData["Message"] = "Đăng ký tham gia sự kiện thành công!";

            // Các logic kiểm tra lại trạng thái sự kiện lần cuối (bảo mật)
            var suKien = _context.SuKien.Find(maSK);
            int daDangKy = _context.DangKy.Count(d => d.MaSK == maSK);

            if (suKien.TrangThai != "Chưa diễn ra" || daDangKy >= suKien.max_participants)
            {
                TempData["ErrorMessage"] = "Sự kiện hiện không cho phép đăng ký.";
                return RedirectToAction("ChiTietSK", "Home", new { id = maSK });
            }
